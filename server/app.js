...
app.post('/api/applications/:userId', async (req, res) => {
    const {userId} = req.params;
    const {name, startDate, payment} = req.body;
    await pool.execute('INSERT INTO applications (user_id, name, start_date, payment) VALUES (?, ?, ?, ?)', [userId, name, new Date(startDate), payment]);
    return res.status(200).send();
});

app.put('/api/applications/:appId/review', async (req, res) => {
    const {appId} = req.params;
    const {review} = req.body;
    await pool.execute('UPDATE applications SET review = ? WHERE id = ?', [review, appId]);
    res.status(200).send();
})
